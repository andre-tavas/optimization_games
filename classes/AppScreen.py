...
class AppScreen:
    ...
    def game_loop(self, game_fun):
        self.screen.update()
        self.screen.listen(None, 'Return')
        self.keep_game_over = True
        self.screen.clear()
        self._split_screen()

        if self._optimal:
            score_1, score_2, optimal_obj = game_fun(self, get_optimal=self._optimal, num_cities=10)
            self.show_result(score_1, score_2, optimal_obj)
        else:
            score_1, score_2 = game_fun(self, num_cities=10)
            self.show_result(score_1, score_2)

        turtle.listen()
        self.screen.onkey(lambda: self.game_loop(game_fun), 'Return')
        self.screen.update()
    ...
    def _show_instructions(self):
        # TODO: colocar o laco de repeticao para comecar nessa funcao
    ...
    def wait_for_start(self, fun):
        self.show_instructions()
        self.screen.listen()
        self.screen.onkey(lambda: self.start_game(fun, self), 't')

    def start_game(self, fun):
        self.screen.clear()
        self.screen.onkey(None, 't')
        fun(self.screen)
        self.wait_for_start(fun, self)
    ...
    def show_result(self, score_1, score_2):
